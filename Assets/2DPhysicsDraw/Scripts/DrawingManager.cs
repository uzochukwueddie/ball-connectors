...
public class DrawingManager : MonoBehaviour
{
    ...
    [HideInInspector]
    public bool canDraw = true; // enables or disables the drawing when clicked on some environment object,
    ...
    // public float transformOffsetY, transformOffsetX;
    ...
    void FixedUpdate()
    {
        mousePointer.GetComponent<TargetJoint2D>().target = (Vector2)Camera.main.ScreenToWorldPoint(Input.mousePosition) + new Vector2(transform.position.x, transform.position.y + offset);
    }

    void Update()
    {
        if (!GamePlay.instance.gameIsOver) {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if (!IsPointerOverUIObject()){
                    ...
                    if (canDraw == true)
                    {
                        if (GamePlay.instance.gameHasStarted == true) {
                            ButtonPressedSound.instance.PlayDrawSound();
                        }

                        mousePointer.transform.position = Camera.main.ScreenToWorldPoint(Input.mousePosition) + new Vector3(transform.position.x, transform.position.y + offset);
    ...
    void StarPrefab()
    {
        ...
        clone.transform.position = new Vector3(0, 0, 1f);
